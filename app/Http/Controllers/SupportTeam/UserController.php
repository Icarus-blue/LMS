...
class UserController extends Controller
{
    ...
    public function __construct(UserRepo $user, LocationRepo $loc, MyClassRepo $my_class, MessageRepo $message_repo)
    {
    ...
    public function update(UserRequest $req, $id)
    {
        ...
        if($req->hasFile('photo')) {
            ...
            $f = Qs::getFileMetaData($photo);
            $f['name'] = $id."." . $f['ext'];
            $f['path'] = $photo->storeAs("admission_number/", $f['name']);
            $data['photo'] = $f['name'];
        }
        if($req->hasFile('sign')) {
            $sign = $req->file('sign');
            $f = Qs::getFileMetaData($sign);
            $f['name'] = $id."." . $f['ext'];
            $f['path'] = $sign->storeAs("sign_number/", $f['name']);
            $data['sign'] = $f['name'];
        }
        // if($req->hasFile('photo')) {
        //     $photo = $req->file('photo');
