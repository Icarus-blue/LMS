...
{
    protected $my_class, $user, $teacher_repo, $message_repo;
    ...
    public function index()
    {
        ...
        if ($d['teacher'] == null) {
            $d['is_check_having_subject_teacher'] = 0;
            $d['is_check_having_class'] = 0;
            $d['is_check_having_form'] = 0;
        } else {
            $teacher_id = $d['teacher']->id;
            $d['is_check_having_subject_teacher'] = count($this->teacher_repo->is_check_having_subject_teacher($teacher_id));
            $d['is_check_having_class'] = count($this->teacher_repo->is_check_having_class($teacher_id));
            $d['is_check_having_form'] = count($this->teacher_repo->is_check_having_form($teacher_id));
        }
    ...
    public function delete_selected_student(Request $req)
    {
        ...
        return redirect()->action(
            [MyClassController::class, 'class_manage1'],
            ['form_id' => $req->formId]
        );
    }
    public function delete_selected_student1(Request $req)
    {
        ...
        //return redirect()->route('class_manage1/{form_id}',['form_id'=>$req->form_id]);
        return redirect()->action(
            [MyClassController::class, 'students_taken_form'],
            ['form_id' => $req->formId]
        );
    }
    ...
    public function delete_selected_subject(Request $req)
    {
        ...
        // return ;
        return redirect()->action(
            [MyClassController::class, 'students_taken_csubject'],
            ['classSubjectId' => $req->subject]
        );
    }
