...
// MOVE STUDENT ACTION
    ...
    function submitFileImport(myObj) {
        ...
        console.log($(myObj).prop("files")[0])
        var ajaxOptions = {
            url:'file-import2',
            type:'POST',
            cache:false,
        ...
        req.done(function(resp){
            ...
            resp.ok && resp.res
            ? flash({msg:resp.res, type:'success'})
            : flash({msg:resp.res, type:'danger'});
            hideAjaxAlert();
            window.location.href = "http://localhost:8000/students";
            return resp;
        });
        req.fail(function(e){
            if (e.status == 422){var errors = e.responseJSON.errors; displayAjaxErr(errors);}
    ...
    function clear_field() {
       window.location.href = "http://localhost:8000/clear_field"
    }
    ...
    $("#key_in_student_details").on('click',()=>{
        console.log(1);
        $("#first_panel").show();
        $("#second_panel").hide();
    })

    $("#upload_from_sheet").on('click',()=>{
        console.log(2);
        $("#second_panel").show();
        $("#first_panel").hide();
    })
