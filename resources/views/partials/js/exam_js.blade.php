                    ...
                    for (let ef of resp.examforms) {
                        if (entry.id == ef.exam_id) { // exams.id == examforms.exam_id
                            ...
                            if (flagexamname == false) {
                                bb += `<td rowspan="` + rr + `" style='height:55px;text-align:left'>` + entry
                                    .name + `</td>`; //exams.name
                                flagexamname = true;
                            }
                            for (let f of resp.forms) {
                                if (f.id == ef.form_id) bb += `<td style='height:55px ;text-align:left'>Form ` +
                                    f.name + `</td>`; // form.id == examforms.form_id
                            }
                            if (ef.flag) {
                            ...
                            if (ef.flag) {
                                ...
                                bb += `<td align="center" class=action` + ef.exam_id + ef.form_id + ` style='height:55px;padding:5px !important'>
                                                <a class="btn d-flex flex-row" href="/exam_manage/config/` + ef
                                    .exam_id + `/` + ef.form_id + `" style="color:#172b4c !important; background-color:#e6e6e6; border-color: #999; width: 140px;padding:10px;font-size:16px;border-radius:10px">
                                                    <i _ngcontent-are-c166="">
                            ...
                            if (!ef.flag) {
                                bb +=
                                    `<td align="center" style='height:55px;padding:5px !important' class=action` +
                                    ef.exam_id + ef.form_id + `>
                                                        <a class="btn" href="/exam_manage/publish/` + ef.exam_id +
                                    `/` + ef.form_id + `"  style="font-size:13px;border-radius:7px;color:white; background-color:#152D4e; width: 140px; display: flex; flex-direction: row;padding:4px" >
                                ...
                                //                     </a>
                                //         </td>`;
                                bb += `<td class=action` + ef.exam_id + ef.form_id +
                                    ` style='height:55px;padding:5px !important'  align="center">
                                ...
                                bb += `<td class=action` + ef.exam_id + ef.form_id + ` align="center" style='height:55px;padding:2px !important'>
                                                <button type="button" class="btn send_result" onclick="SendRequest(` +
                                    ef.exam_id + `, ` + ef.form_id +
                                    `);"  >
                                                ...
                                                <button type="button" class="btn un_publish_result" onclick="unpublish(` + ef.exam_id + `, ` + ef
                                    .form_id + `);" >
                                                    <span>
                                                        <div class="d-flex">
...
function onEditExam(i, id, type, name, term, year, myObj) {
    ...
    if (edit_status == true) {
        ...
        origin_ss[5].innerHTML = `<button type="button" class="btn" onclick="onEditExam('` + origin_i + `', '` +
            origin_id + `', '` + origin_type + `', '` + origin_name + `', '` + origin_term + `', '` + origin_year + `', this);" style="background: white;">
...
function onCancel(myObj) {
    ...
    ss[5].innerHTML = `<button type="button" class="btn" onclick="onEditExam('` + origin_i + `', '` + origin_id +
        `', '` + origin_type + `', '` + origin_name + `', '` + origin_term + `', '` + origin_year + `', this);" style="background: white;">
                            <img src="global_assets/images/icon/edit.png" width=25 height=25 />
                        </button>
                        <button type="button" class="btn" onclick="onDeleteExam('` + origin_id + `', this);" style="background: white;" >
                            <img src="global_assets/images/icon/delete.png" width=25 height=25 />
...
function onUpdate(myObj) {
    ...
    req.done(function(resp) {
        ...
        ss[5].innerHTML = `<button type="button" class="btn" onclick="onEditExam('` + origin_i + `', '` +
            post_id + `', '` + post_type + `', '` + post_name + `', '` + post_term + `', '` + post_year + `', this);" style="background: white;">
                                <img src="global_assets/images/icon/edit.png" width=25 height=25 />
                            </button>
        ...
        if (num == 0) {
            ...
            Swal.fire({
                ...
                if (res.isConfirmed) {
                    ...
                    var exam_name = exam_name_input_elem.value;
                    var exam_term = exam_term_select_elem.options[exam_term_select_elem.selectedIndex]
                        .value;
                    var exam_year = exam_year_select_elem.options[exam_year_select_elem.selectedIndex]
                        .value;
...
function editPaperRatio(id, myObj) {
    ...
    td1.innerHTML =
        `<div class="d-flex">
                            <button class="btn mx-2 cancel-class-stream" style="padding: 6px;" onclick="cancelPaperRatio(` + id +
        `,this);">
...
function selectExam() {
    ...
    req.done(function(res) {
        ...
        for (let my_class_sub of res.my_class_subject) {
            flag = 0;
            if ((res.types == "student" || res.types == "staff" || res.types == "teacher" || res.types ==
                    "admin")) {
                k++;
                pp = 0;
                cc += ` <tr style="line-height:0.5">
                                <td style="font-family: 'Times New Roman', Times, serif">` + k + `</td>
                                <td style="font-family: 'Times New Roman', Times, serif">Form` + my_class_sub.my_class
                    .form.name + " " + my_class_sub.my_class.stream + " - " + my_class_sub.subject.title +
                    ` </td>`;
                ...
                for (let exam_record of my_class_sub.subject.exam_record) {
                    if (exam_record.my_class_id == my_class_sub.my_class_id && exam_record.exam_id == exam
                        .value && exam_record.af == my_class_sub.subject_id) {
                        console.log("checking ......", exam_record.flag);
                        if (exam_record.flag == null) {
                            flag = 1;
                            cc +=
                                `<td class="text-success" style="font-family: 'Times New Roman', Times, serif;">Uploaded but not published by <span style="color:red">subject teacher</span></td>`;
                        } else if (exam_record.flag == 1) {
                            flag = 2;
                            cc +=
                                `<td class="text-success" style="font-family: 'Times New Roman', Times, serif;">Pending publishing by <span  style="color:red">class teacher</span></td>`;
                        } else if (exam_record.flag == 2) {
                            flag = 2;
                            cc +=
                                `<td class="text-success" style="font-family: 'Times New Roman', Times, serif;">Pending publishing by <span style="color:red">class supervisor</span></td>`;
                        } else if (exam_record.flag == 3) {
                            flag = 2;
                            cc +=
                                `<td class="text-success" style="font-family: 'Times New Roman', Times, serif;">Pending publishing by <span style='color:red'>Dean</span></td>`;
                        } else if (exam_record.flag == 4) {
                            flag = 2;
                            cc +=
                                `<td class="text-success" style="font-family: 'Times New Roman', Times, serif;">Published by <span style='color:red'>Dean</span></td>`;
                        }
                ...
                if (pp > 0) {
                    if (flag == 0) {
                        cc +=
                            `<td class="text-danger" style="font-family: 'Times New Roman', Times, serif;"> Upload Results</td>`;
                    }
                } else {
                    cc +=
                        `<td class="text-danger" style="font-family: 'Times New Roman', Times, serif;"> This Class did not sit for this exam</td>`;
                }
                if (pp > 0) {
                    cc +=
                        `<td class="text-right" style="padding-right:5px !important;font-family: 'Times New Roman', Times, serif;">`;
                    if (flag == 1) {
                        cc += `<a href="exam_class_upload/publish/` + my_class_sub.id + `/` + exam.value + `/` +
                            my_class_sub.teacher_id + `/` + my_class_sub.subject.id +
                            `" class="btn btn-success px-2" style="font-family: 'Times New Roman', Times, serif;margin-top:2px;margin-bottom:2px;padding-left:1.3rem;padding-right:1.3rem;border-radius:5px;">Publish</a>`

                    } else if (flag == 2) {
                        cc += `<a href="exam_class_upload/view/` + my_class_sub.id + `/` + exam.value + `/` +
                            my_class_sub.teacher_id + `/` + my_class_sub.subject.id +
                            `" class="btn btn-success" style="background: #b7c1d1;color: #172b4c;margin-top:2px;margin-bottom:2px;padding-left:1.3rem;padding-right:1.3rem;border-radius:5px;">View</a>`
                    } else {
                        cc += `<a href="exam_class/upload/` + my_class_sub.id + `/` + exam.value + `/` +
                            my_class_sub.teacher_id + `/` + my_class_sub.subject.id + `" class="btn btn-info" style="background: #b7c1d1;color: #172b4c;margin-top:2px;margin-bottom:2px;padding-left:10px;padding-right:1.3rem;border-radius:5px;"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"  style = "float:left;margin-right:8px;margin-top:3px" fill="currentColor" class="bi bi-cloud-upload" viewBox="0 0 16 16">
                    ...
                    cc += "</td><td class='d-none'>" + my_class_sub.id + "</td><td class='d-none'>" + exam
                        .value + "</td><td class='d-none'>" + my_class_sub.teacher_id +
                        "</td><td class='d-none'>" + my_class_sub.subject.id + "</td></tr>"
                } else {
        ...
        for (let myclass of res.all_myclasses) {
            if (myclass.teacher_id == res.teacher.id) {
                ...
                cc += `
                    <tr>
                        <td style="font-family: 'Times New Roman', Times, serif;">` + k + `</td>
                        <td style="font-family: 'Times New Roman', Times, serif;">Form` + myclass.form.name + ` ` +
                    myclass.stream + `</td>
                    `;
                cc +=
                    `<td class="text-danger" style="font-family: 'Times New Roman', Times, serif;">View Results</td>`;
                cc +=
                    `<td class="text-right" style="padding-right:5px !important;font-family: 'Times New Roman', Times, serif;">`;
                cc += `<a  href = "exam_stream_view/` + res.teacher.id + `/` + myclass.id + `/` + exam.value +
                    `" class="btn btn-info" style="font-family: 'Times New Roman', Times, serif;background: #b7c1d1;color: #172b4c;margin-top:2px;margin-bottom:2px;padding-left:1.3rem;padding-right:1.3rem;border-radius:5px;">View</a>`
                cc += "</td><td class='d-none'>" + myclass.id + "</td><td class='d-none'>" + myclass
                    .teacher_id + "</td></tr>"
            }
        }
        ...
        for (let form of res.forms) {
            if (form.teacher_id == res.teacher.id) {
                ...
                cc += `
                    ...
                    `;
                cc +=
                    `<td class="text-danger"  style="font-family: 'Times New Roman', Times, serif;text-align:center">View Results</td>`;
                cc +=
                    `<td class="text-right" style="padding-right:5px !important;font-family: 'Times New Roman', Times, serif">`;
                cc += `<a href="exam_form_view/` + form.id +
                    `" class="btn btn-info" style="font-family: 'Times New Roman', Times, serif;background: #b7c1d1;color: #172b4c;margin-top:2px;margin-bottom:2px;padding-left:1.3rem;padding-right:1.3rem;border-radius:5px;">View</a>`
                cc += "</td><td class='d-none' style='font-family: 'Times New Roman', Times, serif;'>" + form
                    .id + "</td><td class='d-none'>" + res.teacher.id + "</td></tr>"
            }
        }
...
function addMarkItem(e) {
    ...
    req.done(function(resp) {
        ...
        var row = `<tr id=row` + rowCount + `>
                <td>
                    <select class="form-control" id="adm_no` + rowCount + `" onchange="selUser(this, ` + rowCount +
            `)"><option></option>`;
                    ...
                    <div class="input-group">
                        ...
                        <span class="input-group-text">
                            <div class="dropdown">
                                ...
                                <i class="icofont-trash" id="mark_new_trash` + rowCount +
            `" onclick="removeNewContent(` + rowCount + `)" style="cursor: pointer;display:none"></i>
                            </div>
                        </span>
                    </div>
                </td>
                <td><button type="button" class="btn btn-secondary" id=ID` + rowCount + ` onclick="removeForRow(` +
            rowCount + `)">Delete</button></td>
...
function newSave(e) {
    ...
    for (let index = 0; index < srcRowCount; index++) {
        if ($(`input[name=src_mark` + index + `]`).val() > 0) {
            ...
            console.log(exam_id, $('#std' + index).text(), class_id, $(`input[name=src_mark` + index + `]`).val(),
                p_comment, af, section_id);
            var ajaxOptions = {
                url: $('#exam_publish_mark').attr('action'),
                type: 'POST',
                cache: false,
