...
async function changeSignature() {
    ...
    if (files != null) {
        contentSignElement.innerHTML = files.map(file =>
            `<img src="${URL.createObjectURL(file)}" style="width: 200px; height: 50px;">`).join('');
        flash({
            msg: "Your signature photo has been successfully updated",
            type: 'success'
        })
    }
}
...
function imageSignDelete() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            flash({
                msg: "Your signature photo has been successfully deleted",
                type: 'danger'
            })
        }
    });
...
function imagePhotoDelete(e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            contentPhotoElement.innerHTML =
                `<img class="mt-3 rounded-circle" src="/global_assets/images/user.png" style="width: 100px; height: 100px;">`;
            flash({
                msg: "Your profile photo has been successfully deleted",
                type: 'danger'
            })
        }
    });
...
async function changeImage(e) {
    ...
    let files = await selectFile("image/*", true);
    if (files != null) {
        contentPhotoElement.innerHTML = files.map(file =>
            `<img class="mt-3 rounded-circle" src="${URL.createObjectURL(file)}" style="width: 100px; height: 100px;">`
            ).join('');
        flash({
